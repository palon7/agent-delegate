...
import { configPath, DEFAULT_SANDBOX, isAgent, readConfig, setSandbox, } from "./lib/config.mjs";
...
try {
    ...
    if (values.help) {
        console.log("config.mjs [--agent opencode|codex --sandbox srt|none] (change only at the user's request)");
    }
    else if (values.agent !== undefined || values.sandbox !== undefined) {
        assertNotDelegated();
        if (!isAgent(values.agent) ||
            !["srt", "none"].includes(values.sandbox ?? ""))
            throw new Error("Required: --agent opencode|codex --sandbox srt|none");
        setSandbox(values.agent, values.sandbox);
