...
export function checkSrtVersion(cli) {
    ...
    if (result.error ||
        ...
        throw new Error(`SRT ${SRT_VERSION} is required at ${cli}: ${result.error ?? (result.stderr.trim() || result.stdout.trim() || `exit ${result.status}`)}`);
}
...
export function checkSrtExecution(cli, settings, cwd, env) {
    ...
    try {
        ...
        if (result.error || result.status !== 0 || result.stdout !== marker)
            throw new Error(`SRT preflight failed before agent launch: ${result.error ?? (result.stderr.trim() || result.stdout.trim() || `exit ${result.status}`)}`);
    }
    finally {
