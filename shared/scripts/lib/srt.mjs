...
export function checkSrtVersion(cli) {
    if (!fs.existsSync(cli))
        throw new Error(`SRT is missing: ${cli}. Run prepare.mjs for the installation plan.`);
    // 0.0.76's CLI reports npm_package_version or a hardcoded 1.0.0.
    // Read the installed package instead of trusting that banner.
    const metadata = path.resolve(path.dirname(fs.realpathSync(cli)), "..", "package.json");
    if (fs.existsSync(metadata)) {
        ...
        if (pkg.name === "@anthropic-ai/sandbox-runtime") {
            if (pkg.version !== SRT_VERSION)
                throw new Error(`SRT ${SRT_VERSION} is required at ${cli}; found ${pkg.version}`);
            return;
        }
    }
    ...
    if (result.error ||
        ...
        throw new Error(`SRT ${SRT_VERSION} is required at ${cli}: ${result.error ?? (result.stderr.toString().trim() || result.stdout.toString().trim() || `exit ${result.status}`)}`);
}
...
export function checkSrtExecution(cli, settings, cwd, env) {
    ...
    try {
        ...
        if (result.error ||
            ...
            throw new Error(`SRT preflight failed before agent launch: ${result.error ?? (result.stderr.toString().trim() || result.stdout.toString().trim() || `exit ${result.status}`)}`);
    }
    finally {
        fs.rmSync(temporary, { recursive: true, force: true });
    }
}
export function validateProfile(file) {
    ...
    if (!profile ||
        ...
        throw new Error(`Invalid SRT profile: ${file}; network.allowedDomains and filesystem.allowRead are required`);
}
