...
export function executableCommand(file, args = []) {
    const node = /\.[cm]?js$/i.test(file) ||
...
export function resolveExecutable(value, npmPackage) {
    const windows = process.platform === "win32";
    const explicit = path.isAbsolute(value) ||
        value.includes("/") ||
    ...
    let unresolvedWrapper;
    for (const candidate of candidates) {
        const files = windows && !path.extname(candidate)
            ? [
                candidate + ".exe",
                candidate + ".com",
                candidate + ".cmd",
    ...
    if (unresolvedWrapper)
        throw new Error(`Cannot resolve script wrapper: ${unresolvedWrapper}. Specify the agent's .exe or Node entry point instead.`);
...
function npmEntry(wrapper, packageName) {
    const dir = path.dirname(wrapper);
    const modules = path.basename(dir) === ".bin"
        ? path.dirname(dir)
    ...
    if (pkg.name !== packageName || typeof bin !== "string")
        return;
