...
`prepare.mjs --agent codex --new-job` creates only a job directory under `/tmp` on POSIX systems or the user's temporary directory on Windows. Launch creates persistent runtime logs, SQLite storage, and runtime temporary directories. Preparation reports the existing `codex_home`. Do not create or authenticate a separate Codex home.
