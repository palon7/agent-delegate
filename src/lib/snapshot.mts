...
export function git(
  ...
  args: string[],
  {
    ...
    data,
    allowed = [0],
  }: { env?: NodeJS.ProcessEnv; data?: Buffer; allowed?: number[] } = {},
): Buffer {
...
export function gitText(...args: Parameters<typeof git>): string {
  return git(...args)
    .toString()
    .trim();
}
...
function snapshotEnvironment(directory: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
function readSnapshotInfo(directory: string): SnapshotInfo {
  return JSON.parse(
...
export function initialize(
  ...
  const snapshot = path.join(
    fs.realpathSync(path.dirname(path.resolve(directory))),
...
export function capture(directory: string, label: string): string {
  ...
  for (const record of splitNulTerminated(
    git(cwd, ["ls-files", "--stage", "-z"], { env }),
  )) {
    const tab = record.indexOf(9);
    modes.set(
      record.subarray(tab + 1).toString("hex"),
      record.subarray(0, 6).toString(),
    );
  }
  const fileMode =
    gitText(cwd, ["config", "--bool", "--get", "core.filemode"], {
      allowed: [0, 1],
    }) !== "false";
  const symlinks =
    gitText(cwd, ["config", "--bool", "--get", "core.symlinks"], {
      allowed: [0, 1],
    }) !== "false";

  const entries = candidatePaths(cwd, include, env).map((name) =>
    indexEntry(
      cwd,
      name,
      include,
      env,
      modes.get(name.toString("hex")),
      fileMode,
      symlinks,
    ),
  );
...
export function difference(directory: string): Buffer {
  const snapshot = fs.realpathSync(directory);
  ...
  return git(
    cwd,
    ["diff", "--no-ext-diff", "--no-textconv", before, after, "--"],
    { env: snapshotEnvironment(snapshot) },
  );
}
...
function candidatePaths(
  ...
  const changed = git(
    cwd,
    ["diff-files", "--name-only", "--no-ext-diff", "--no-textconv", "-z"],
    { env },
  );
  const untracked = git(
    cwd,
  ...
  for (const name of [
    ...
    if (name.length) paths.set(name.toString("hex"), name);
  return [...paths.values()].sort(Buffer.compare);
}

function splitNulTerminated(buffer: Buffer): Buffer[] {
  const names: Buffer[] = [];
  let start = 0;
  for (
...
function indexEntry(
  ...
  if (path.isAbsolute(text) || text.split("/").includes(".."))
    throw new Error(
      "included paths must be relative files inside the repository",
    );

  const absolute = Buffer.concat([Buffer.from(cwd + "/"), name]);
  const stat = lstatIfExists(absolute);
  if (!stat) return removalEntry(name);
  ...
  if (stat.isFile()) {
    if (!symlinks && trackedMode === "120000")
      return entry(
        "120000",
        storeObject(cwd, env, ["--stdin"], fs.readFileSync(absolute)),
        name,
      );
...
function entry(mode: string, oid: string, name: Buffer): Buffer {
...
function removalEntry(name: Buffer): Buffer {
  return entry("0", NULL_OID, name);
}

function lstatIfExists(file: Buffer): fs.Stats | undefined {
  try {
...
function quotePath(file: Buffer): Buffer {
  const escaped = [...file]
