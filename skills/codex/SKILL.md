---
name: codex
description: Delegate implementation, review, or another explicitly requested task to a separate Codex CLI session. Use when the user invokes this skill or asks for Codex delegation; not for ordinary coding or questions about Codex.
---
...
- Reuse handler state and the explicit saved child session ID for corrections. Do not use `--last`, the parent session, or another repository's session. The runner disables Agent Delegate and further subagent delegation, retains its depth checks, and removes parent task/session IDs from the child's environment. Other plugins and tools keep their normal configuration.
